    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-2 md:col-span-2">
            <label className="text-sm font-medium text-slate-700">แหล่งข้อมูล (Data Source Link)</label>
            <input 
              type="url" 
              name="data_source"
          ...
          <div className="space-y-2 md:col-span-2">
            <label className="text-sm font-medium text-slate-700">หมวดหมู่ 4+1 Excellence</label>
            <select 
              name="excellence_category"
              value={formData.excellence_category || ''}
              onChange={handleChange}
              className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all bg-white"
            >
          ...
          <div className="space-y-4 md:col-span-2 p-4 bg-slate-50 rounded-2xl border border-slate-100">
            <h4 className="text-lg font-bold text-slate-700 flex items-center gap-2">
              <i className="fa-solid fa-bullseye text-indigo-500"></i>
              เป้าหมายตามรอบการประเมิน (Targets)
            </h4>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-700">รอบ 6 เดือน</label>
                <input 
                  type="text" 
                  value={formData.targets?.['6_months'] || ''}
                  onChange={(e) => setFormData(prev => ({
                    ...prev,
                    targets: { ...prev.targets, '6_months': e.target.value }
                  }))}
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-700">รอบ 9 เดือน</label>
                <input 
                  type="text" 
                  value={formData.targets?.['9_months'] || ''}
                  onChange={(e) => setFormData(prev => ({
                    ...prev,
                    targets: { ...prev.targets, '9_months': e.target.value }
                  }))}
                  placeholder="เช่น ร้อยละ 70"
                  className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all"
                />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-700">รอบ 12 เดือน</label>
                <input 
                  type="text" 
                  value={formData.targets?.['12_months'] || ''}
                  onChange={(e) => setFormData(prev => ({
                    ...prev,
                    targets: { ...prev.targets, '12_months': e.target.value }
                  }))}
                  placeholder="เช่น ร้อยละ 100"
                  className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all"
                />
              </div>
            </div>
