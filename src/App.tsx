...
import { MOUDashboard } from './components/dashboard/MOUDashboard';
...
import { WorkGroupManagement } from './components/admin/WorkGroupManagement';
...
import { PrivacyPolicyModal, POLICY_VERSION } from './components/common/PrivacyPolicyModal';
import { Calendar } from './components/dashboard/Calendar';
import { PrivacySettings } from './components/settings/PrivacySettings';
import { sortIndicators } from './lib/utils';
...
import { sendTelegramNotification, formatVerificationMessage } from './lib/telegram';
...
export default function App() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (indicators && indicators.length > 0) {
        ...
        setData(sortIndicators(formattedData));
      } else {
        setData([]);
      }
      ...
      if (!settingsError && settingsData) {
        const enabledSetting = settingsData.find(s => s.key === 'data_entry_enabled');
        const startSetting = settingsData.find(s => s.key === 'start_date');
        const endSetting = settingsData.find(s => s.key === 'end_date');

        let isEnabled = enabledSetting?.value === 'true';
        ...
        if (isEnabled) {
          ...
          if (endSetting?.value && new Date(endSetting.value) < now) {
            isEnabled = false;
          }
        }

        setIsDataEntryEnabled(isEnabled);
      }
    } catch (error) {
  ...
  // Realtime subscription for new data submissions
  useEffect(() => {
    if (user?.role !== 'admin' && user?.role !== 'กลุ่มงาน สสจ.') return;
  ...
  const handleSaveMasterIndicator = async (indicator: Indicator) => {
    try {
      ...
      const { order, responsible_groups, targets, data_source, ...rest } = indicator;
      const dbData = {
        ...rest,
  ...
  const handleLogin = (loggedInUser: User) => {
    ...
    // Check privacy policy acceptance and version
    const acceptedVersion = localStorage.getItem(`privacy_accepted_version_${loggedInUser.id}`);
    if (acceptedVersion !== POLICY_VERSION) {
  ...
  const handleAcceptPrivacy = () => {
    if (user) {
      localStorage.setItem(`privacy_accepted_version_${user.id}`, POLICY_VERSION);
  ...
  const handleSettingsPrivacy = () => {
    // Navigate to settings tab after accepting or just close and navigate
    // For now, let's just close and show the app at settings tab
    if (user) {
      localStorage.setItem(`privacy_accepted_version_${user.id}`, POLICY_VERSION);
      setShowPrivacyPolicy(false);
  ...
  const handleVerifyIndicator = async (indicator: Indicator, area: string, status: 'ผ่าน' | 'ไม่ผ่าน' | 'รอประเมิน' | 'รอยืนยัน' | 'แก้ไข', feedback?: string, deadline?: string, ppho_work_group?: string) => {
    try {
      ...
      if (updatedIndicator.results[timeframe] && updatedIndicator.results[timeframe][area]) {
        ...
        if (status === 'แก้ไข') {
          updatedIndicator.results[timeframe][area].rejection_time = new Date().toISOString();
          updatedIndicator.results[timeframe][area].rejection_deadline = deadline;
          updatedIndicator.results[timeframe][area].ppho_work_group = ppho_work_group;
        } else {
          delete updatedIndicator.results[timeframe][area].rejection_time;
          delete updatedIndicator.results[timeframe][area].rejection_deadline;
          delete updatedIndicator.results[timeframe][area].ppho_work_group;
        }
        ...
        const historyEntry = {
          ...
          feedback: feedback,
          deadline: deadline,
          ppho_work_group: ppho_work_group,
      ...
      const { error } = await supabase
        .from('indicators')
        .update({ 
          results: updatedIndicator.results,
          updated_at: new Date().toISOString()
        })
        .eq('id', indicator.id);
      ...
      // Send Telegram Notification for verification results
      if (status === 'ผ่าน' || status === 'แก้ไข' || status === 'ไม่ผ่าน') {
        try {
  ...
  // Track pendingCount changes to show popup
  const [prevPendingCount, setPrevPendingCount] = useState(0);
  
  useEffect(() => {
    if ((user?.role === 'admin' || user?.role === 'กลุ่มงาน สสจ.') && pendingCount > prevPendingCount && prevPendingCount !== 0) {
      Swal.fire({
        title: 'มีข้อมูลใหม่รอการตรวจสอบ!',
        text: 'มีการส่งข้อมูลตัวชี้วัดใหม่เข้ามา กรุณาตรวจสอบ',
        icon: 'info',
        showCancelButton: true,
        confirmButtonText: 'ไปตรวจสอบ',
  ...
  }, [pendingCount, prevPendingCount, user?.role]);
        ...
        <AnimatePresence>
          {showPrivacyPolicy && (
            <PrivacyPolicyModal 
              ...
              onSettings={handleSettingsPrivacy}
            />
          )}
        </AnimatePresence>
      </>
      ...
      <div className="flex-1 flex flex-col min-w-0 transition-all duration-300">
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto w-full">
          ...
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'dashboard' && selectedModule === 'mou' && (
              <motion.div
                key="mou-dashboard"
              ...
              >
                <MOUDashboard 
                  ...
                  isDataEntryEnabled={isDataEntryEnabled}
                />
              </motion.div>
            )}
            ...
            {activeTab === 'my_dashboard' && (
              <motion.div
                ...
                transition={{ duration: 0.2 }}
              >
                <CustomDashboard data={data} timeframe={timeframe} user={user} />
              </motion.div>
            )}

            {activeTab === 'executive_dashboard' && user?.role === 'ผู้บริหาร' && (
              <motion.div
                ...
                transition={{ duration: 0.2 }}
              >
                <ExecutiveDashboard data={data} timeframe={timeframe} />
              </motion.div>
            )}
            ...
            {activeTab === 'verify' && (user?.role === 'admin' || user?.role === 'กลุ่มงาน สสจ.' || user?.role === 'ผู้บริหาร') && (
              <motion.div
                key="verify"
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
            ...
            {activeTab === 'workgroups' && user?.role === 'admin' && (
              <motion.div
                key="workgroups"
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
            ...
            {activeTab === 'system_settings' && user?.role === 'admin' && (
              <motion.div
                key="system_settings"
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
